// 全局變量 - 使用安全的存取方式避免重複宣告
// 不再顯式宣告全局變量，而是使用window對象的安全存取

// 確保關鍵全局對象存在
window.policyData = window.policyData || {};
window.currentModule = window.currentModule || 'basic';
window.showMarketComparison = window.showMarketComparison || false;

// 定義便於存取的局部引用 - 使用函數而非直接賦值
function getPolicyData() { return window.policyData; }
function getCurrentModule() { return window.currentModule; }
function getShowMarketComparison() { return window.showMarketComparison; }
...
function calculateAndCompare() {
    ...
    setTimeout(() => {
        try {
            ...
            // 移除必填限制，允许不完整的数据
            if (isNaN(ownPremium)) ownPremium = 1000000; // 默认值100万
            if (isNaN(ownRate)) ownRate = 2.5; // 默认值2.5%
            if (isNaN(ownSurrender)) ownSurrender = ownPremium * 1.2; // 默认值为保费的1.2倍
...
// 檢測是否為 iPad - 使用不同名稱以避免衝突
function checkIsiPadDevice() {
